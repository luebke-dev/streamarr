...
# Usage:     curl -fsSL https://get.streamarr.media | sudo bash
#            or:  sudo bash install.sh
# =============================================================================
set -euo pipefail

STREAMARR_VERSION="1.0.0"
INSTALL_DIR="/opt/streamarr"
...
BANNER
    echo -e "${NC}"
    echo -e "  ${CYAN}Installer v${STREAMARR_VERSION}${NC}"
    echo ""
}
...
# Streamarr docker-compose — generated by installer
services:
  ...
  db:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER:-streamarr}"]
      interval: 10s
      timeout: 5s
      retries: 5
...
COMPOSE_ES
  ...
  migrate:
    image: \${REGISTRY:-registry.gitlab.com/streamarr.media}/backend:\${IMAGE_TAG:-latest}
    command: ["uv", "run", "alembic", "upgrade", "head"]
    restart: "no"
    env_file: .env
    environment:
      - DATABASE_URL=postgresql+asyncpg://\${POSTGRES_USER:-streamarr}:\${POSTGRES_PASSWORD}@db:5432/\${POSTGRES_DB:-streamarr}
      - SECRET_KEY=\${SECRET_KEY}
    depends_on:
      db:
        condition: service_healthy
  ...
  backend:
    image: \${REGISTRY:-registry.gitlab.com/streamarr.media}/backend:\${IMAGE_TAG:-latest}
    command: ["uv", "run", "uvicorn", "streamarr.web:app", "--host", "0.0.0.0", "--port", "8000", "--workers", "\${BACKEND_WORKERS:-2}"]
    restart: unless-stopped
    privileged: true
    env_file: .env
    environment:
...
$(echo -e "$backend_depends")
  ...
  worker:
    image: \${REGISTRY:-registry.gitlab.com/streamarr.media}/backend:\${IMAGE_TAG:-latest}
    command: ["uv", "run", "taskiq", "worker", "streamarr.worker:broker"]
    restart: unless-stopped
    privileged: true
    env_file: .env
    environment:
...
$(echo -e "$es_env")
  ...
  scheduler:
    image: \${REGISTRY:-registry.gitlab.com/streamarr.media}/backend:\${IMAGE_TAG:-latest}
    command: ["uv", "run", "taskiq", "scheduler", "streamarr.worker:scheduler"]
    restart: unless-stopped
    env_file: .env
    environment:
      - DATABASE_URL=postgresql+asyncpg://\${POSTGRES_USER:-streamarr}:\${POSTGRES_PASSWORD}@db:5432/\${POSTGRES_DB:-streamarr}
...
$(echo -e "$es_env")
  ...
  # ── Frontend ──────────────────────────────────────────────
  frontend:
    image: \${REGISTRY:-registry.gitlab.com/streamarr.media}/frontend:\${IMAGE_TAG:-latest}
    restart: unless-stopped
    ports:
...
COMPOSE_SAB
  ...
  # ── Lightrays (Game Streaming) ────────────────────────────
  lightrays:
    image: ${REGISTRY:-registry.gitlab.com/streamarr.media}/lightrays:${IMAGE_TAG:-latest}
    restart: unless-stopped
    privileged: true
    network_mode: host
    environment:
    ...
    volumes:
      ...
      - ${LIGHTRAYS_HOST_STATE_DIR:-/var/lib/streamarr/lightrays}:/state/lightrays:rw
...
setup_systemd() {
    ...
    cat > /etc/systemd/system/streamarr.service << EOF
[Unit]
...
EOF
    ...
    systemctl enable streamarr.service
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  streamarr.media installed successfully!${NC}"
    echo -e "${GREEN}${BOLD}============================================================${NC}"
    echo ""
    ...
    echo -e "    ${CYAN}systemctl status streamarr${NC}                   # Systemd status"
    echo ""
...
main() {
    ...
    echo ""
    info "This script will install streamarr.media and its dependencies."
    echo ""
