...
class NetworkRuntimeConfig:
    ...
    def restart_required(self) -> bool:
        return (
            os.environ.get("PYRATE_EFFECTIVE_BIND_HOST") not in (None, self.bind_host)
            or os.environ.get("PYRATE_EFFECTIVE_BIND_PORT") not in (
                None,
                str(self.bind_port),
            )
            or os.environ.get("PYRATE_EFFECTIVE_ENABLE_HTTPS") not in (
                None,
    ...
    def reverse_proxy_env(self) -> dict[str, str]:
        return {
            "PYRATE_UPSTREAM_HOST": self.bind_host,
            "PYRATE_UPSTREAM_PORT": str(self.bind_port),
            "PYRATE_UPSTREAM_SCHEME": self.scheme,
            "PYRATE_PUBLIC_BASE_URL": self.public_base_url or "",
            "PYRATE_INTERNAL_BASE_URL": self.internal_base_url,
        }
