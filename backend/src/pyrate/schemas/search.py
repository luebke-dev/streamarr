...
# ==================== Indexer Release Search Schemas ====================


class SeriesTypeEnum(StrEnum):
    """Series type used to drive different search strategies."""

    STANDARD = "standard"  # S01E01 format
    DAILY = "daily"  # Date-based format (late-night shows)
    ANIME = "anime"  # Absolute episode numbering


class EpisodeSearchRequest(BaseSchema):
    """Request for an indexer episode search."""

    episode_guid: uuid.UUID = Field(..., description="Episode guid")
    force_refresh: bool = Field(
        default=False, description="Force a fresh indexer search"
    )


class SeasonSearchRequest(BaseSchema):
    """Request for a season-pack search."""

    show_guid: uuid.UUID = Field(..., description="Show guid")
    season_number: int = Field(..., ge=0, description="Season number")


class ShowReleaseSearchRequest(BaseSchema):
    """Request for a show-wide release search."""

    show_guid: uuid.UUID = Field(..., description="Show guid")
    query: str | None = Field(default=None, description="Optional search term")


class IndexerReleaseResult(BaseSchema):
    """A single release result from an indexer."""

    # Basic info
    title: str = Field(..., description="Release title")
    indexer_id: str = Field(..., description="Indexer id")
    link: str | None = Field(default=None, description="Download link")
    size: int = Field(default=0, description="File size in bytes")

    # Parsed info
    parsed_title: str = Field(default="", description="Parsed show title")
    parsed_season: int | None = Field(
        default=None, description="Parsed season number"
    )
    parsed_episode: int | None = Field(
        default=None, description="Parsed episode number"
    )
    parsed_absolute_episode: int | None = Field(
        default=None, description="Parsed absolute episode number"
    )
    quality: str = Field(default="", description="Parsed quality")

    # Scoring
    score: float = Field(default=0.0, description="Score")
    match_type: str = Field(
        default="unknown", description="Match type: exact, season_pack, daily, anime"
    )

    # Additional indexer metadata
    pub_date: datetime | None = Field(
        default=None, description="Publish date"
    )
    category: str | None = Field(default=None, description="Category")
    grabs: int | None = Field(default=None, description="Download count")
    seeders: int | None = Field(default=None, description="Seeder count (torrent)")
    peers: int | None = Field(default=None, description="Peer count (torrent)")


class IndexerSearchResponse(BaseSchema):
    """Response for an indexer search."""

    results: list[IndexerReleaseResult] = Field(
        default_factory=list, description="Releases found"
    )
    ...
    indexer_count: int = Field(default=0, description="Number of indexers queried")

    # Search context
    show_title: str = Field(default="", description="Show title")
    season_number: int | None = Field(default=None, description="Season number")
    episode_number: int | None = Field(default=None, description="Episode number")
    series_type: SeriesTypeEnum = Field(
        default=SeriesTypeEnum.STANDARD, description="Series type"
    )

    # Scene titles used
    scene_titles: list[str] = Field(
        default_factory=list, description="Scene titles used"
    )
