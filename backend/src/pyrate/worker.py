...
from pyrate.services.task_events import (
    record_worker_task_event,
)
from pyrate.services.trending import TrendingService
from pyrate.workers.cleanup import StorageCleanupWorker
from pyrate.workers.downloads import DownloadRefreshWorker
from pyrate.workers.playback import TranscodeMonitorWorker
from pyrate.workers.runtime import broker, create_scheduler
from pyrate.workers.mass_operation_worker import (
    run_mass_operation_rule_impl,
    tick_mass_operations_impl,
)
from pyrate.workers.overlay_worker import (
...
async def backfill_age_ratings() -> None:
    ...
    from pyrate.utils.age_rating import parse_min_age
...
async def import_book(openlibrary_id: str) -> None:
    ...
    try:
        async with sessionmanager.session() as db:
            from pyrate.services.book_import import BookImportService
...
async def send_notification_email(notification_id: str) -> None:
    ...
    try:
        async with sessionmanager.session() as db:
            ...
            from pyrate.services.auth import _get_email_i18n, _get_app_url
            from pyrate.services.settings import SettingsService

            i18n = _get_email_i18n("notification", user)
            lang = (user.ui_language or "en-US")[:2].lower()
            try:
                app_name = await SettingsService(db).get("system.site_name", "Pyrate Media")
            except Exception:
...
async def probe_media_file(media_file_guid: str) -> dict:
    ...
    try:
        ...
        from pyrate.services.play import probe_video_full
                ...
                for task in all_tasks:
                    ...
                    if not session_id or session_id not in active_session_ids:
                        ...
                        try:
                            ...
                            if session_id:
                                from pyrate.services.storage_cleanup import cleanup_session_temp_files
...
async def run_upgrade_search(media_item_guid: str) -> dict:
    ...
    from pyrate.services.upgrade_scan import run_upgrade_search_impl
