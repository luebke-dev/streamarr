...
from pyrate.services.installation import InstallationService
...
class InitialSetupRequest(BaseModel):
    ...
    site_name: str | None = Field(default="pyrate.media", max_length=100)
...
async def get_installation_status(db: DatabaseSession):
    ...
    if has_users:
        ...
        site_name = await settings_service.get("system.site_name", "pyrate.media")
    ...
    **Security Note**: This endpoint is only available before the first user is created.
    ...
    try:
        ...
        return InitialSetupResponse(
            ...
            site_name=setup_data.site_name or "pyrate.media",
        )
...
async def get_install_wizard_configuration(
    ...
    return InstallWizardConfiguration(
        server_name=await settings_service.get("system.site_name", "pyrate.media"),
