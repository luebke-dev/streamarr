...
from streamarr.utils.host_path import sanitize_bind_host_path as _sanitize_bind_host_path
...
class DockerComputingProvider(ComputingBase):
    ...
    async def start_task(
        ...
        **kwargs,
        ...
        config["Labels"]["streamarr.task_id"] = task_id
    ...
    async def list_tasks(
        ...
        # Build label filter
        filters = {"label": ["streamarr.task_id"]}
        if labels:
        ...
        try:
            ...
            for container in containers:
                ...
                task_id = container_labels.get("streamarr.task_id")
    ...
    async def cleanup_completed_tasks(self, max_age_hours: int = 24) -> int:
        ...
        try:
            containers = await self.client.containers.list(
                all=True, filters={"label": ["streamarr.task_id"]}
            )
            for container in containers:
                info = await container.show()
                state = info["State"]
