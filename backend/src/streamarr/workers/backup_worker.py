...
def backup_dir() -> Path:
    """Directory pg_dump archives are written to (``STREAMARR_BACKUP_DIR``)."""
    return Path(os.environ.get("STREAMARR_BACKUP_DIR", _DEFAULT_BACKUP_DIR))
...
async def run_pg_dump_backup(reason: str = "scheduled") -> dict:
    ...
    out_file = out_dir / f"streamarr-db-{ts}.dump"
