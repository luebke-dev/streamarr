...
Chromecast Cast V2, AirPlay) live in :mod:`streamarr.services.casting`. This
...
from streamarr.services.casting import (
...
from streamarr.services.websocket import RemoteControlError, get_websocket_manager
...
# a user's own ``streamarr`` receiver session). Enumerating/controlling these is
...
async def discover_cast_targets(
    ...
    """Return manual targets plus active streamarr receiver sessions for this user."""
    items: list[CastDiscoveryTarget] = [
        CastDiscoveryTarget(
            **target.model_dump(),
            discovered=False,
    ...
    for active_session in manager.get_active_device_sessions(str(current_user.guid)):
        ...
        if not device_id:
            continue
        target_id = f"streamarr:{device_id}"
...
async def list_cast_protocol_capabilities(
    ...
    return [
        CastProtocolCapability(
            protocol="streamarr",
            display_name="streamarr receiver",
            ...
            notes="Active streamarr WebSocket receiver sessions can be discovered and controlled.",
        ),
        CastProtocolCapability(
            protocol="chromecast",
...
async def send_cast_target_command(
    ...
    """Send a command to a cast target backed by a connected streamarr device."""
    target = next(
        (
            target
            for target in await _load_targets(session)
    ...
    # drive their own connected ``streamarr`` receiver sessions below.
    ...
    elif target.protocol == "streamarr" and target.device_id:
        try:
