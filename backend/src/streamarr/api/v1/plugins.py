...
def _core_notification_event_types() -> set[str]:
    from streamarr.api.v1.notifications import EVENT_DEFINITIONS
        ...
        if plugin.runtime_kind != "metadata":
            runtime_warnings.append(
                PluginValidationIssue(
                    ...
                    severity="warning",
                    message=(
                        "Runtime metadata is recorded for admins only; streamarr does not "
...
async def get_plugin_runtime_policy(
    ...
    return PluginRuntimeSummary(
        policy=PluginRuntimePolicy(
            notes=(
                "streamarr currently stores plugin repository, lifecycle, UI/static, "
...
async def validate_plugin_metadata(
    ...
    """Validate stored plugin metadata against streamarr's safe runtime contract."""
