...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run streamarr API server")
    parser.add_argument("--reload", action="store_true")
    args = parser.parse_args()

    config = asyncio.run(_load_runtime_config())

    os.environ["STREAMARR_EFFECTIVE_BIND_HOST"] = config.bind_host
    os.environ["STREAMARR_EFFECTIVE_BIND_PORT"] = str(config.bind_port)
    os.environ["STREAMARR_EFFECTIVE_ENABLE_HTTPS"] = str(config.enable_https).lower()
    ...
    uvicorn.run("streamarr.web:app", **uvicorn_kwargs)
