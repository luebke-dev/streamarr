...
def _message_task_id(message: Any, labels: dict) -> str:
    ...
    raw_task_id = labels.get("streamarr_task_id") or labels.get("task_id") or task_name
    return str(raw_task_id or "unknown")


def _message_category(task_id: str, labels: dict) -> str:
    raw_category = labels.get("streamarr_category") or labels.get("category")
    if raw_category:
...
def _message_run_id(message: Any, labels: dict) -> str:
    ...
    try:
        labels["streamarr_run_id"] = run_id
