...
module (``streamarr.services.search``): Redis import locks, the background
...
from streamarr.schemas.search import SearchType

logger = logging.getLogger(__name__)

# Redis key prefix / TTL for import locks. Defined here (single source of
# truth) and re-exported by streamarr.services.search for the public API.
IMPORT_LOCK_PREFIX = "streamarr:import_lock:"
...
class SearchImportQueueMixin:
    ...
    Mixed into :class:`streamarr.services.search.SearchService`.
    """
    ...
    async def _import_igdb_game(self, external_id, media_service) -> Any:
        ...
        from streamarr.models.media import AvailabilityStatus, MediaType
    ...
    async def _import_spotify_artist(self, external_id, media_service) -> Any:
        ...
        try:
            from streamarr.worker import import_album as import_album_task
    ...
    async def _import_spotify_song(self, external_id) -> dict[str, str] | None:
        ...
        from streamarr.worker import import_album as _sync_import_album
