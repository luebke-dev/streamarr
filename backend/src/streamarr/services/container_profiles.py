...
def _clean_mounts(value: Any) -> list[dict[str, Any]]:
    ...
    an absolute path, are dropped **silently** — Streamarr validates only the
...
def _games_library_host_root() -> str:
    ...
    project_root = os.environ.get("PROJECT_ROOT", "/root/streamarr.media").rstrip("/")
...
async def available_platforms(db: AsyncSession, media_item: Any) -> list[dict[str, Any]]:
    ...
    """
    from streamarr.models.media import MediaItem, MediaRelease
...
async def resolve_launch_config(
    ...
    if selected_platform:
        ...
        from streamarr.services.game_platforms import profile_for_platform
