...
* :class:`streamarr.services.provider_search.ProviderSearchMixin` — provider
  transforms and per-provider search calls.
* :class:`streamarr.services.search_import_queue.SearchImportQueueMixin` — import
...
client classes …) remain importable from ``streamarr.services.search``.
"""
...
from streamarr.services.provider_search import TRANSIENT_SEARCH_ERRORS, ProviderSearchMixin
from streamarr.services.search_import_queue import (
    IMPORT_LOCK_PREFIX,
...
class SearchService(ProviderSearchMixin, SearchImportQueueMixin):
    ...
    async def browse_local(
        ...
        from streamarr.services.local_search import LocalSearchService
    ...
    def _resolve_hit_provider(hit: dict, fallback_type: "SearchType | None" = None):
        ...
        Requires ``from streamarr.models.media import MediaType`` at call site;
