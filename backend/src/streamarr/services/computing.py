...
from streamarr.computing.base import ComputingBase
from streamarr.computing.docker import DockerComputingProvider
from streamarr.computing.kubernetes import KubernetesComputingProvider
...
_PVC_LIBRARY = lambda: os.environ.get("STREAMARR_PVC_LIBRARY", "streamarr-library")  # noqa: E731
_PVC_DOWNLOADS = lambda: os.environ.get("STREAMARR_PVC_DOWNLOADS", "streamarr-downloads")  # noqa: E731
_PVC_CACHE = lambda: os.environ.get("STREAMARR_PVC_CACHE", "streamarr-cache")  # noqa: E731
...
def build_media_volumes(
    ...
    * **Kubernetes** — PVC names, mounted once per volume at the same
      ...
      ``streamarr-library`` PVC carries every library subdirectory under
      ...
      node. Volume names are overridable via ``STREAMARR_PVC_*`` envs for
...
class ComputingService:
    ...
    async def start_ffmpeg_task(
        ...
        task_labels = {
            "streamarr.task_type": "transcode",
            "streamarr.tool": "ffmpeg",
            **(labels or {}),
        }
    ...
    async def start_ffprobe_task(
        ...
        task_labels = {
            "streamarr.task_type": "probe",
            "streamarr.tool": "ffprobe",
            **(labels or {}),
        }
    ...
    async def probe_media_file(
        ...
        :func:`streamarr.libraries.base.structure_probe_data`).
