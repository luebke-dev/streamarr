...
#
# Reads from ``LIGHTRAYS_SESSION_TIMEOUT_SECS`` so Streamarr's bookkeeping
# ages out on the same schedule as the Lightrays-side idle reaper. A
# small ``GRACE`` is added so Streamarr doesn't prune a session that the
...
async def launch_session(
    *,
    ...
    bind the session to the authenticated Streamarr user. Capability, device,
    ...
    override and is intended for values already validated by Streamarr.
...
#
# Layout:
#   streamarr:lightrays:user:{user_id}        (sorted set; score = started_ts,
#                                            member = session_id)
#   streamarr:lightrays:session:{session_id}  (hash; user_id, media_id,
#                                            started_ts) — TTL refreshed
#                                            on writes


def _user_set_key(user_id: str) -> str:
    return f"streamarr:lightrays:user:{user_id}"


def _session_key(session_id: str) -> str:
    return f"streamarr:lightrays:session:{session_id}"
...
async def get_session_record(session_id: str) -> dict | None:
    """Return Streamarr's Redis metadata for a Lightrays session."""
    from streamarr.services.rate_limiter import _get_redis
