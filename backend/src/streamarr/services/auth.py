...
from streamarr.config import get_app_url, settings
...
class AuthService:
    ...
    async def _get_site_name(self) -> str:
        """Load site name from DB settings, fallback to 'Streamarr'."""
        from streamarr.services.settings import SettingsService

        try:
            return await SettingsService(self.db).get("system.site_name", "Streamarr")
        except Exception:
            return "Streamarr"

    async def _send_welcome_email(self, user: User, app_name: str = "Streamarr") -> bool:
    ...
    async def _send_verification_email(self, user: User, app_name: str = "Streamarr") -> bool:
    ...
    async def _send_password_reset_email(self, user: User, app_name: str = "Streamarr") -> bool:
    ...
    def _validate_registration_input(
        ...
        if password and settings.oidc.local_auth_enabled:
            from streamarr.auth.password import validate_password_strength
    ...
    async def reset_password(self, token: str, new_password: str) -> dict[str, str]:
        ...
        from streamarr.auth.token_revocation import revoke_user_refresh_tokens
